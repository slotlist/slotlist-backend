...
export const missionSlotSchema = Joi.object().keys({
    ...
    difficulty: Joi.number().min(0).max(4).required().description('Difficulity of the slot, ranging from 0 (easiest) to 4 (hardest)').example(4),
    ...
    assignee: userSchema.allow(null).required().description('User the slot has been assigned to. Can be null if no final assignment has been completed yet')
